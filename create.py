...
def get_obj_diagonal(obj):
	return round(math.sqrt(obj.width*2 + obj.height*2))


def mutate_image(img):
	# resize image for random value
	#resize_rate = random.choice(sizes)
	resize_rate = random.uniform(0.3,1.2)
...
if __name__ == "__main__":
	...
	for bkg in bkg_images:
		...
		for i in obj_images:
			...
			random_image_count = random.randint(2, 7)
			for j in range(0, random_image_count):
				# Copy background
				...
				if args.mutate:
					...
					w = new_obj.width
					h = new_obj.height
					max_x = bkg_w_obj.width - w
					max_y = bkg_w_obj.height - h
